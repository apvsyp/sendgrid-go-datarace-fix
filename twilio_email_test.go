...
func TestNewTwilioEmailSendClient(t *testing.T) {
	...
	req, err := mailClient.prepareRequest(&mail.SGMailV3{}, nil)
	assert.NoError(t, err)
	assert.Equal(t, "https://email.twilio.com/v3/mail/send", req.BaseURL)
	assert.Equal(t, "Basic dXNlcm5hbWU6cGFzc3dvcmQ=", req.Headers["Authorization"])
}
